...
5. Converters:
   - `pyvips_converter.py` — preferred high-performance path using `pyvips`; passes HEIF through untouched and renders DNG to 10-bit HEIF
...
- **No-re-encode fast paths**: files already in the target container (JPEG, HEIF, MP4) are copied or remuxed rather than re-encoded; because no encoder runs, ExifTool strips the metadata instead of `strip=True` (HEIF needs `-all=`).
